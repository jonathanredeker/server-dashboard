...
function DashboardToolbar(props) {
    ...
    // Dropdown pagination
    if (props.page > 1) {
        dropdowns.push(<Dropdown.Item href={`/${props.page - 1}/${props.limit}`}>Page {props.page - 1}</Dropdown.Item>)
    }
    dropdowns.push(<Dropdown.Item href={`/${props.page}/${props.limit}`}>Page {props.page}</Dropdown.Item>)
    if (props.page < props.totalNodes / props.limit) {
        dropdowns.push(<Dropdown.Item href={`/${props.page + 1}/${props.limit}`}>Page {props.page + 1}</Dropdown.Item>)
    }

    return (
        <div className="row ml-4 mr-4 pl-3 pr-3 mt-5 d-flex justify-content-end">
            <div className="col-sm-auto mt-2 d-flex justify-content-end">
